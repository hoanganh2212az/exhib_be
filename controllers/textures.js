...
export const get = async (req, res, next) => {
  try {
    ...
    if (texture_id) {
      ...
      const data = await tb.getById(req.accessToken, "textures", texture_id);
      return res.json(data);
    }

    // ========= GET ALL (NO TOKEN) =========
    if (!hasToken) {
      const data = await tb.allItems(
        "textures",
        "*",
...
export const remove = async (req, res, next) => {
  try {
    ...
    // gọi tb.deleteById cho từng id
    for (const id of ids) {
      await tb.deleteById(req.accessToken, "textures", id);
    }
