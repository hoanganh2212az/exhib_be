...
export const get = async (req, res, next) => {
  try {
    ...
    if (room_id) {
      ...
      // with token -> RLS
      const data = await tb.getById(req.accessToken, "rooms", room_id);
      return res.json(data);
    }

    // ========= GET ALL (NO TOKEN) =========
    if (!hasToken) {
      const data = await tb.allItems("rooms", "*", (q) => q.range(from, to));
      return res.json(data);
    }

    // ========= GET ALL (WITH TOKEN / RLS) =========
    const data = await tb.listItems(req.accessToken, "rooms", "*", (q) =>
      q.range(from, to)
    );
...
export const update = async (req, res, next) => {
  try {
    ...
    const updated = await tb.updateById(
      req.accessToken,
      "rooms",
      req.query.room_id,
      req.body
...
export const remove = async (req, res, next) => {
  try {
    ...
    for (const id of ids) {
      await tb.deleteById(req.accessToken, "rooms", id);
    }
