...
const fileFilter = (req, file, cb) => {
  ...
  const ok = (ext === '.glb') && (file.mimetype === 'model/gltf-binary' || file.mimetype === 'application/octet-stream');
  if (!ok) return cb(new Error('Only .glb is allowed (model/gltf-binary).'));
  cb(null, true);
};
export const uploadModel = multer({ storage, limits: { fileSize: MAX_SIZE }, fileFilter }).single('model');

export const list = async (req, res, next) => {
  try {
    const data = await tb.listItems(req.accessToken, 'object3d', '*', (q) => q);
    res.json(data);
  } catch (err) { next(err); }
};
...
export const create = async (req, res, next) => {
  try {
    ...
    const up = await supabaseAdmin.storage
      .from('object3d')
      .upload(filepath, req.file.buffer, { contentType: 'model/gltf-binary', upsert: false });

    if (up.error) throw up.error;

    // 4) Lấy URL (nếu bucket public: dùng publicUrl; nếu private: có thể đổi sang signed URL)
    const { data: pub } = supabaseAdmin.storage.from('object3d').getPublicUrl(filepath);
    ...
    const data = await tb.insertItem(req.accessToken, 'object3d', payload);
...
export const update = async (req, res, next) => {
  try {
    ...
    const data = await tb.updateById(req.accessToken, 'object3d', req.params.id, changes);
    res.json(data);
  } catch (err) { next(err); }
};
...
export const remove = async (req, res, next) => {
  try {
    const data = await tb.deleteById(req.accessToken, 'object3d', req.params.id);
    res.json(data);
  } catch (err) { next(err); }
};
